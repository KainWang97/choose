...
- [x] RWD 商品詳情頁：產品顏色和尺寸選項移至商品敘述上方
- [x] RWD 管理面板：修正全面排版問題（含客服訊息區塊跑版）
- [x] 管理面板按鈕語義：「Edit」改「編輯」，「規格」改「庫存管理」

**相關檔案**：`ProductDetail.vue`, `AdminDashboard.vue`
...
- [ ] 郵件通知優化 (非同步、失敗處理)
- [ ] 用戶端詢問歷史紀錄
- [ ] **RWD 商品詳情頁優化**：產品顏色和尺寸選項移至商品敘述上方
- [ ] **RWD 管理面板排版修復**：修正客服訊息區塊跑版問題
- [ ] **管理面板按鈕語義調整**：將商品「Edit」改為「編輯」，「規格」改為更符合功能的名稱
